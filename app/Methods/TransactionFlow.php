...
class TransactionFlow{
    ...
    public static function updateInTransactionFlow ($request,$id) {
       ...
       $transaction = Transaction::find($id);
        ...
        $previous_approver = array();
        
        if (!is_null($previous_voucher_transaction['transaction_voucher']) && $previous_voucher_transaction['transaction_voucher']->count() > 0) {
        ...
        if (isset($request['voucher']['month']) && !is_null($request['voucher']['month']) && !is_null($previous_voucher_month)) {
            $voucher_month = GenericMethod::with_previous_transaction($request['voucher']['month'], $previous_voucher_month);
        }
